            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center mt-4 gap-2">
                ...
                <div class="d-flex align-items-center gap-3">
                    <span class="text-muted small">
                        <b>{{ $alumni->firstItem() }}</b> - <b>{{ $alumni->lastItem() }}</b> of <b>{{ $alumni->total() }}</b>
                    </span>
                    <nav>
                        <ul class="pagination pagination-sm mb-0">
                            {{-- Previous --}}
                            <li class="page-item {{ $alumni->onFirstPage() ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $alumni->previousPageUrl() }}{{ $alumni->previousPageUrl() ? '&per_page='.request('per_page', 10) : '' }}" tabindex="-1">
                                    <i class="fa-solid fa-chevron-left"></i>
                                </a>
                            </li>
                            {{-- Page Numbers --}}
                            @foreach ($alumni->getUrlRange(1, $alumni->lastPage()) as $page => $url)
                                @if ($page == $alumni->currentPage() || ($page <= 2 || $page > $alumni->lastPage() - 2 || abs($page - $alumni->currentPage()) <= 1))
                                    <li class="page-item {{ $alumni->currentPage() == $page ? 'active' : '' }}">
                                        <a class="page-link" href="{{ $url }}&per_page={{ request('per_page', 10) }}">{{ $page }}</a>
                                    </li>
                                @elseif ($page == 3 && $alumni->currentPage() > 4)
                                    <li class="page-item disabled"><span class="page-link">...</span></li>
                                @elseif ($page == $alumni->lastPage() - 2 && $alumni->currentPage() < $alumni->lastPage() - 3)
                                    <li class="page-item disabled"><span class="page-link">...</span></li>
                                @endif
                            @endforeach
                            {{-- Next --}}
                            <li class="page-item {{ !$alumni->hasMorePages() ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $alumni->nextPageUrl() }}{{ $alumni->nextPageUrl() ? '&per_page='.request('per_page', 10) : '' }}">
